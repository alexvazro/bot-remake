...
client_mongo = pymongo.MongoClient("mongodb+srv://alex:j7WLycrEfzVhiYw8@botcluster.aiwbw2u.mongodb.net/?retryWrites=true&w=majority")
...
def hard_fix(client):
  ...
  for x in collections:
    ...
    if (grand_found == False) or (counter_found == False):
      ...
      mydict = { '_id': item_id, "author": author, "counter": counter_value, "grand": grand_value }
...
def hard_fix_products(client):
  ...
  for x in collections:
    ...
    #if something is missing, delete it and create a new one
    if (purr_found == False) or (kitten_found == False) or (woman_found == False):
      myquery = { "_id": item_id }
      products_db.delete_one(myquery)
      

      mydict = { '_id': item_id, "author": author, "Get purred": purr_value, "Kitten": kitten_value, "Woman": woman_value }
