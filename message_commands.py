  ...
  async def on_message(message):
    ...
    if msg.startswith("!buy"):
      if len(msg) == 4:
        ...
        print("ok")
        item = msg[5:]
        to_print = shop.main_shop(str(message.author),item, client)
        await message.channel.send(to_print)

    if msg.startswith("!collection"):
      value1, value2, value3 = functions_db.show_collection(str(message.author))

      title = str(message.author)[:-5] + "'s " + "collection"

      embedVar = discord.Embed(title=title, color=0x98FFEF)
      embedVar.add_field(name="Times you got purred", value=value1, inline=False)
      embedVar.add_field(name="Kittens groomed", value=value2, inline=False)
      embedVar.add_field(name="Women molested", value=value3, inline=False)
    ...
    if msg.startswith("!roles"):
      ...
      if members == role:
        ...
        embedVar.add_field(name='‎', value=string_to_print, inline=False)
        await message.channel.send(embed=embedVar)
        return
    ...
    if msg == "!help":
      ...
      embedVar.add_field(name="!help", value="Shows all commands available", inline=False)
      embedVar.add_field(name="!attention", value="For all you attention seeking whores", inline=False)
      embedVar.add_field(name="!buy", value="Buy something from the !shop  \n  EG: !buy Kitten", inline=False)
      embedVar.add_field(name="!collection", value="Shows your collection the items you've bought", inline=False)
      embedVar.add_field(name="!counter", value="Displays how many bad words you have said", inline=False)
      embedVar.add_field(name="!monke", value="Treats you with a random monkey gif", inline=False)
      embedVar.add_field(name="!points", value="Shows how many points you have" + '\n' + "1 message = 1 point", inline=False)
      embedVar.add_field(name="!roles", value="This displays all the admin members \n EG: !roles admin team", inline=False)
      embedVar.add_field(name="!shop", value="View the things you can buy with your points", inline=False)
      embedVar.add_field(name="!top", value="Shows the top 3 of people who've sworn the most", inline=False)
      embedVar.add_field(name="!top points", value="Shows the top 3 of people who have the most points", inline=False)
      await message.channel.send(embed=embedVar)
      return
    ...
    if (message.channel.id == 944362633820270593) and ((msg[0] != "#") or (len(msg) != 7)):
      await message.delete()

    if message.channel.id == 947448160941400134:
      l = len(msg)
      if (msg[l - 2:] != "||") or (msg[:2] != "||") or (len(msg) <= 4):
        await message.delete()
